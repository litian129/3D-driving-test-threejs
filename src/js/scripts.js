...
import { YELLOWVEHICLESPATHS } from './constants';
import { REDVEHICLESPATHS } from './constants';
import { BLUEVEHICLESPATHS } from './constants';
import { ANSWERSTEXT } from './constants';
import { WHEELS } from './constants';
import { BLINKINGLIGHTS, LISTENER, AUDIOS } from './constants';
 ...
 * 换句话说，它显著缩小了 3D 图形文件的大小，同时对 3D 图形的观看者来说又根本不严重影响视觉效果。它还旨在改善 3D 图形的压缩和传输。 
 */
...
function changeOptionText(qtion, opt1, opt2, opt3) {
...
loader.load('assets/arrow.glb', (glb) => {
    ...
    createArrow(new THREE.Vector3(5.91, 2, -161.04), -0.5*Math.PI)
    createArrow(new THREE.Vector3(-82.82, 2, -171.17), -Math.PI)
    ...
    createArrow(new THREE.Vector3(-98.74, 2, -166.74), -Math.PI)
...
function showAnswerSymbol(opt1, opt2, opt3) {
    console.log('option1Symbol :>> ', option1Symbol);
